...
description -- bootup (register modules, fetch config, querystring, goto init, subscribe)
...
diagrams: bootstrap-1, 2 and 3 (general, config, run)

FIXME methods of palSettings are not defined yet here.
The configuration is encapsulated in the \texttt{palSettings} angular service.
...
Angular attaches an object \texttt{\$scope} to all directives.
A scope can be isolated, prototypically inherited from the parent, or shared with other directives of the same type.
Values exposed to the view are stored in the scope.
...
diagram with the sample xml and the properties object

Flango \cm is designed to adapt to this syntax and to decouple and reuse elements as much as possible.
Directives can be restricted to Element, Attribute, Class or coMment.
...
The stylesheet of the theme defines the inheritance of styles: a base-button is implemented with a button and, therefore, inherits all properties of the class $button$.

$<fl:ui base-button></ui>$ eventually becomes $<div class="base-button" style="..."><div class="caption">Hello World</caption></div>$.
Properties are exclusive for the UI Component they are attached to: they are interpreted as in-line \ac{CSS} in the \texttt{style} \ac{HTML} attribute.

With this rewrite strategy the directive eventually writes real \ac{HTML} that defines the structure of the UI Component and uses the stylesheet
