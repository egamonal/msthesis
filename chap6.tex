...
\subsection{HTML5}
\ac{HTML} has been extended towards enabling application development in the browser.
This project extensively uses regular \ac{HTML} and the \lstinline$<video>$ tag to implement the UI Video component.
Therefore, new videos are encoded to be compatible with the browser instead of \texttt{flv} and \texttt{f4v}. For example, \texttt{H.264} or \texttt{ogv}.
...
Flango \cm is a component in a system that works with \ac{ROS}, which is similar to a \ac{SOA} system, and interoperates with it using its message system.
The following pages have a high-level description of the system's architecture and the patterns used.
...
Controllers expose values and behaviour via the \$scope service but do not manipulate the \ac{DOM} or read it in any way.
...
One of the \acs{SOLID} principles, \emph{dependency inversion}, states that one should "Depend upon Abstractions. Do not depend upon concretions."
...
\paragraph{Factory} Directives, controllers and services are registered in modules. 
...
\paragraph{Remote Facade} 
The software of this thesis is a component in \ac{ROS}.
A way of integrating it with the system would be desinging the business layer as a service\footnote{This approach was only considered in the last weeks of development due to changes in robotBehaviour and only a simplified design is completed.}.
A Remote Facade subscribes and listens to \ac{ROS} topics and exposes the logic to other components.
Normally, remote facades have coarse-grained methods and, in this design, operations attend requests made via \ac{ROS} topics to behave like a simplified \emph{service layer}.
...
The technology, however, seems to satisfy the needs of the project.
...
\section{Static View}
This section contains a comprehensive description of the objects in the system and their relationships taking into account the project's technology: JavaScript and Angular client-side, Django in the backend, \ac{ROS} Topics to interoperate with the rest of the system.
...
It can be used as a class-based language using constructors but this project does not need to do this: it does not create objects of any domain-specific class.
Instead, Angular constructs are used to instantiate services, controllers and directives, the building blocks of the application.
Most of the boxes in these diagrams are not strictly classes but objects that the framework instantiates using the provided definitions.
...
The concept of package is implemented with Angular modules.
...
They are grouped in 6 categories: boot-strap and configuration, handling properties and transformation to \ac{HTML}, internal navigation, and communication with ROS Bridge.
It always works the same way: writing the appropriate behaviour in directives (\texttt{controller}, \texttt{compile} and \texttt{link} functions) and injecting the required services.
...
It is created with a factory: when a new instance of \texttt{palSettings} is to be created, it runs some code that prepares the instance that is returned.
During the bootstrap process, there is a call to \lstinline$palSettings.init(queryString)$ (\fref{fig:design-seqdia-palSettings-init}).
...
\begin{enumerate}
\item \textbf{Generic configuration} (\fref{fig:design-seqdia-palSettings-getGenericConfig}). Paths, \ac{API} endpoint, etc. 
\item \textbf{Application specific configuration} (\fref{fig:design-seqdia-palSettings-getApplicationConfig}).  Available languages, available themes, default language, etc.
\item \textbf{Structure} (\fref{fig:design-seqdia-palSettings-getStructure}). A graph that matches screens with \ac{URI}. 
\end{enumerate}

It fails gradually: it first tries to fetch the real application. If that fails, it attempts to fetch the default application (\fref{fig:design-seqdia-palSettings-defaultApplicationConfig}). If this is not possible, it obtains the static application shipped with the program (See \texttt{errorCallback} in \fref{fig:design-seqdia-palSettings-defaultApplicationConfig}).
...
\paragraph{Inheritance and composition} UI Components can be composited: a \texttt{home-button} is implemented with a \texttt{base-button}, which in turn is implemented with a \texttt{button} that eventually becomes \ac{HTML}.
Each component can have default values for properties.
For instance, if \texttt{base-button} has a \texttt{width} of 100px in the template, and a component that is implemented with \texttt{base-button} (e.g. \texttt{home-button}) does not define a different width, it sets \texttt{width} to 100 in the properties object.
...
Actions are declared in the UI Component (listing \ref{actionlist-xml}).
Directives set the corresponding field in the properties object that represents the chain of actions to execute on click.

\lstinputlisting[label=actionlist-xml, language=xml, caption={A list of actions in \ac{XML}}]{src/example-actions.xml}

\lstinputlisting[language=xml, caption={Result button (snippet)}]{src/example-result-button-action.html}

When the component is clicked, it calls \lstinline$executeActions()$, which has a list of actions to execute as a parameter using the \emph{command pattern}.
...
Angular has a navigation system based on \acp{URL}.
The \$route service manages a map that relates \acp{URL} with screens, \ac{HTML} files with directives.
...
The \texttt{palROSBridge} service can be injected to any component that needs to publish to a topic.
palROSBridge.publish(topic, message) uses the roslibjs library to publish the topic.
\Fref{fig:design-seqdia-ActionCtrl-publish} illustrates the interaction between a component and \texttt{palROSBridge} service.

\begin{figure}[htb]
    \centering
    \includegraphics{figures/design/seqdia/actionCtrl-externalCall.pdf}
    \caption{Sequence Diagram: Publishing to a topic}
    \label{fig:design-seqdia-ActionCtrl-publish}
\end{figure}

\FloatBarrier

\subsection{Responding to Requests From The Robot}
The previous section demonstrated how to publish a message to a \ac{ROS} Topic.
This section shows how the facade ROSBridgeCtrl behaves like a service layer and exposes logic to \ac{ROS} (\fref{fig:design-seqdia-ROSBridgeCtrl-creation}): it subscribes to the topic that all components of the robot can use to send data to the Flango \cm .
Whenever a request is received, ROSBridgeCtrl delegates the responsibility to the correct component (\fref{fig:design-seqdia-ROSBridgeCtrl-callback}).
