...
Listing \ref{aaaaa} shows logical nesting and inheritance.

\lstinputlisting[language=xml, caption={SASS code example}]{src/sass-example.css}

\paragraph{Connection with robots} Robots are essentially two Linux boxes with \ac{ssh} enabled.
\reem{H3} has two computers, one in the torso (\texttt{media}) where Flango lives \ref{aaaa}, one in the lower half (\texttt{control}).

\begin{lstlisting}[caption=Connection with robots, label=impl-ssh]
$ ssh pal@reemh3-1m
...
\subsection{Directives and modules}
...
\texttt{service()} returns an object, whereas \texttt{factory()} first creates it and then returns the instance \ref{aaa}
...
\subsection{Transformation to \ac{HTML}}

The \texttt{flUi} directive makes decoupling and reusing directives possible \ref{aaa}.
It removes the type attribute (\texttt{fl:type="base-button"} and creates an attribute with that value (\texttt{base-button}), which can trigger directives (\texttt{flBaseButton}).
...
\lstinputlisting[language=JavaScript, caption={Summary of the flUi directive}]{src/impl-directive-ui.js}
...
flBaseButton (listing \ref{}) is a directive in the themes module.
It only links to a file that contains the template \ref{}.
It is merely a \texttt{button} (a basic UI Component) with certain style set with the \ac{CSS} class \texttt{fl-base-button}.

\lstinputlisting[language=JavaScript, caption={flBaseButton directive}]{src/impl-directive-basebutton.js}
\lstinputlisting[language=xml, caption={flBasebutton directive template}]{src/impl-directive-basebutton-template.xml}

After this template is compiled, Angular finds another directive, \texttt{flButton} \ref{aaa}, which has the logic to create an \ac{HTML} node for the actual button displayed on the screen.

\lstinputlisting[language=JavaScript, caption={flButton directive}]{src/impl-directive-button.js}
...
Essentially, roslibjs has 3 methods: connect, subscribe (and unsubscribe) and publish.
