...
Chapter \fref{chap:testing} has a full list and examples of other test suites that refer directly to components of the software (described in \fref{chap:design})
...
\begin{itemize}
	\item subscreen
	\begin{itemize}
    	\item should always be transformed to \lstinline$<fl:ui fl:subscreen>$
   	 	\item should include a screen file (always uses filename \texttt{screens/99.xml} here) in the inner div
    	\item should add a \lstinline$<div>$ with at least the subscreen css class
	\end{itemize}

	\item img
	\begin{itemize}
    \item should be transformed to \lstinline$<fl:ui fl:img>$
    \item should be replaced with \lstinline$<img>$
    \item should set the ng-src attribute in \lstinline$<img>$
	\end{itemize}

	\item button
	\begin{itemize}
    \item should be transformed to \lstinline$<fl:ui fl:button>$
    \item should be replaced with \\ \lstinline$<a><div class="fl-overlay"></div><div class="caption-container"></div></a>$
	\end{itemize}

	\item textarea
	\begin{itemize}
    \item should be transformed to \lstinline$<fl:ui fl:textarea>$
    \item should be replaced with \lstinline$<div></div>$
	\end{itemize}
